(function(){
  ...
  function execute(context) {
    ...
    //console.log('executing server with context', context);
  ...
  function publicTunnel(port, options){    
    ...
    // ngrok.connect({
    //   addr:port,
    //   subdomain: "adaptabicordova",
    //   authtoken: "5V388cub1QWKNazgdTaEE_4x5Hww8RY5cg6u9ipCyii"
    // }, 
    ngrok.connect(port,
    function (err, url) {
      if (err) {
        console.log('err on ngrok.connect');
        publicTunnelDfd.reject(err);
        return console.log('Could not create tunnel: ', err);
      }

      console.log('public tunnerl says content url is', url);
